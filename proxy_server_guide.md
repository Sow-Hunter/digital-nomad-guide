...
> 脚本不包含 WARP 和分流路由
>
> Xray-script 只负责 Xray 本身的安装和配置。WARP 出口和 AI 服务分流路由需要手动配置，详见第 4-5 章。
...
warp-cli mode proxy          # SOCKS5 代理模式，不接管全局路由
warp-cli proxy port 40000    # 监听端口
warp-cli connect
...
> [!WARNING]
> Xray-script 不会自动安装 WARP
>
> WARP 需要在 Xray-script 安装完成后手动安装。安装顺序：先跑 Xray-script → 再装 WARP → 最后配分流路由。
...
' /usr/local/etc/xray/config.json.bak | sudo tee /usr/local/etc/xray/config.json > /dev/null
...
服务器已把仓库中的 `scripts/generate-ss-chain-link.sh` 安装为 `/usr/local/sbin/generate-ss-chain-link`，所有者为 `root:root`、权限为 `700`。脚本会直接读取当前 Xray 配置中的方法、密码和端口，并通过 AWS IMDS 获取实例公网 IP，因此不会在脚本里复制第二份密码或写死可能变化的地址。

```bash
# 默认自动读取当前实例公网 IP，输出可直接导入客户端的 SIP002 ss:// 链接
sudo /usr/local/sbin/generate-ss-chain-link

# IMDS 不可用或需要给新地址生成链接时，可显式传入地址
sudo /usr/local/sbin/generate-ss-chain-link <OREGON_SERVER_IP>
```
...
TMP_DIR="/tmp/geodata-update"
MIN_SIZE=1048576.   # 1MB - 正常文件至少 5MB+, 低于 1MB 必然异常
...
# 下载到临时目录
curl -sL --max-time 120 --retry 3 -o "$TMP_DIR/geoip.dat" "$GEOIP_URL"
curl -sL --max-time 120 --retry 3 -o "$TMP_DIR/geosite.dat" "$GEOSITE_URL"

# 校验文件大小
geoip_size=$(stat -c%s "$TMP_DIR/geoip.dat" 2>/dev/null || echo 0)
geosite_size=$(stat -c%s "$TMP_DIR/geosite.dat" 2>/dev/null || echo 0)

if [ "$geoip_size" -lt "$MIN_SIZE" ]; then
    echo "[$(date)] ERROR: geoip.dat too small (${geoip_size} bytes), aborting" >> /var/log/geodata-update.log
    exit 1
fi

if [ "$geosite_size" -lt "$MIN_SIZE" ]; then
    echo "[$(date)] ERROR: geosite.dat too small (${geosite_size} bytes), aborting" >> /var/log/geodata-update.log
    exit 1
fi

# 安全替换（校验通过才覆盖）
cp "$TMP_DIR/geoip.dat" "$XRAY_DIR/geoip.dat"
cp "$TMP_DIR/geosite.dat" "$XRAY_DIR/geosite.dat"
...
echo "[$(date)] OK: geoip=${geoip_size} geosite=${geosite_size}" >> /var/log/geodata-update.log
```
...
| --- | --- | --- |
| 下载位置 | 直接下载到目标目录 | 先下载到 `/tmp/geodata-update/` |
| 大小校验 | ❌ 只检查 curl 退出码 | ✅ 文件 < 1MB 直接 abort,不覆盖旧文件 |
| 超时/重试 | ❌ 无 | ✅ `--max-time 120 --retry 3` |
| 失败时旧文件 | 已被 `rm` 删掉,无法回退 | 不动旧文件,直接退出 |
| 日志 | `>/dev/null` 吃掉所有输出 | 写入 `/var/log/geodata-update.log` |
| 临时文件清理 | 手动 `rm` | `trap cleanup EXIT` 自动清理 |
...
# 赋予执行权限
sudo chmod +x /usr/local/xray-script/tool/geodata.sh

# 手动跑一次验证
sudo /usr/local/xray-script/tool/geodata.sh
cat /var/log/geodata-update.log
# 应显示: [日期] OK: geoip=18xxxxxx geosite=10xxxxxx
```
...
> 异常输出形如: `[Fri Jun 27 06:30:01 UTC 2026] ERROR: geoip.dat too small (20 bytes), aborting`（此时 Xray 不受影响,继续用旧文件运行）
